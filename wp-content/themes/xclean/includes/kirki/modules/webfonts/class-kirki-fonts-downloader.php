	...
	protected function get_local_font_styles( $css ) {
		...
		foreach ( $files as $remote => $local ) {
			if( file_exists($local) ){
				...
				// may be domain/server changed so we need to refesh whole
				if( str_contains($local, 'fonts') ){
					$local_array = explode('fonts', $local);
					$new_url = content_url() . '/fonts/' . $local_array[1];
					$files[ $remote ] = $new_url;
				}
			}
		}
		return str_replace(
