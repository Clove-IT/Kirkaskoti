...
class Kirki_Control_Repeater extends Kirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_html__( 'Add new %s', 'xclean' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'			  => '',
								'echo'			  => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'xclean' ),
								'option_none_value' => '0',
								'selected'		  => '',
							)
						);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'xclean' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'xclean' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_html_e( 'No Image Selected', 'xclean' ); ?>
									<# } #>
								</figure>
								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'xclean' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'xclean' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'xclean' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change Image', 'xclean' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'xclean' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'xclean' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_html_e( 'No File Selected', 'xclean' ); ?>
									<# } #>
								</figure>
								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'xclean' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'xclean' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'xclean' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'xclean' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'xclean' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'xclean' ); ?></button>
				</div>
			</li>
		</script>
		<?php
