		...
		<div class="wrapper">
			<# if ( ! _.isUndefined( data.default['font-family'] ) ) { #>
				...
				<div class="font-family">
					<h5><?php esc_html_e( 'Font Family', 'xclean' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-font-family-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'xclean' ); ?>"></select>
				</div>
				<# if ( ! _.isUndefined( data.choices['font-backup'] ) && true === data.choices['font-backup'] ) { #>
					<div class="font-backup hide-on-standard-fonts kirki-font-backup-wrapper">
						<h5><?php esc_html_e( 'Backup Font', 'xclean' ); ?></h5>
						<select {{{ data.inputAttrs }}} id="kirki-typography-font-backup-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'xclean' ); ?>"></select>
					</div>
				<# } #>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant kirki-variant-wrapper">
						<h5><?php esc_html_e( 'Variant', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['font-size'] ) ) { #>
				...
				<div class="font-size">
					<h5><?php esc_html_e( 'Font Size', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['line-height'] ) ) { #>
				...
				<div class="line-height">
					<h5><?php esc_html_e( 'Line Height', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['letter-spacing'] ) ) { #>
				...
				<div class="letter-spacing">
					<h5><?php esc_html_e( 'Letter Spacing', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['word-spacing'] ) ) { #>
				...
				<div class="word-spacing">
					<h5><?php esc_html_e( 'Word Spacing', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['text-align'] ) ) { #>
				...
				<div class="text-align">
					<h5><?php esc_html_e( 'Text Align', 'xclean' ); ?></h5>
					<div class="text-align-choices">
						<input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Inherit', 'xclean' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Left', 'xclean' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Center', 'xclean' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Right', 'xclean' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Justify', 'xclean' ); ?></span>
							</label>
						</input>
					</div>
				</div>
			...
			<# if ( ! _.isUndefined( data.default['text-transform'] ) ) { #>
				...
				<div class="text-transform">
					<h5><?php esc_html_e( 'Text Transform', 'xclean' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-transform-{{{ data.id }}}">
						...
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'xclean' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Capitalize', 'xclean' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Uppercase', 'xclean' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Lowercase', 'xclean' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'xclean' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'xclean' ); ?></option>
					</select>
				</div>
			<# } #>
			<# if ( ! _.isUndefined( data.default['text-decoration'] ) ) { #>
				...
				<div class="text-decoration">
					<h5><?php esc_html_e( 'Text Decoration', 'xclean' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-decoration-{{{ data.id }}}">
						...
						<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'xclean' ); ?></option>
						<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Underline', 'xclean' ); ?></option>
						<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Overline', 'xclean' ); ?></option>
						<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Line-Through', 'xclean' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'xclean' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'xclean' ); ?></option>
					</select>
				</div>
			<# } #>
			<# if ( ! _.isUndefined( data.default['margin-top'] ) ) { #>
				...
				<div class="margin-top">
					<h5><?php esc_html_e( 'Margin Top', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['margin-bottom'] ) ) { #>
				...
				<div class="margin-bottom">
					<h5><?php esc_html_e( 'Margin Bottom', 'xclean' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['color'] ) && false !== data.default['color'] ) { #>
				...
				<div class="color">
					<h5><?php esc_html_e( 'Color', 'xclean' ); ?></h5>
