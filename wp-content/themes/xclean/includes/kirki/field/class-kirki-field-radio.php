...
class Kirki_Field_Radio extends Kirki_Field {
	...
	protected function set_type() {
		...
		if ( in_array( $this->mode, array( 'buttonset', 'image' ), true ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Error in field %1$s. The "mode" argument has been deprecated since Kirki v0.8. Use the "radio-%2$s" type instead.', 'xclean' ), esc_html( $this->settings ), esc_html( $this->mode ) ), '3.0.10' );
