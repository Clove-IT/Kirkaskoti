...
class Kirki_Field {
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Typo found in field %s - setting instead of settings.', 'xclean' ), esc_html( $args['settings'] ) ), '3.0.10' );
		...
		if ( is_array( $config_id ) && empty( $args ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'xclean' ), esc_html( $args['settings'] ), 'https://kirki.org/docs/getting-started/fields.html' ), '3.0.10' );
			$args	  = $config_id;
			$config_id = 'global';
		}
	...
	protected function set_option_type() {
		...
		if ( 'theme_mods' === $this->option_type ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Typo found in field %s - "theme_mods" vs "theme_mod"', 'xclean' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_partial_refresh() {
		...
		foreach ( $this->partial_refresh as $id => $args ) {
			if ( ! is_array( $args ) || ! isset( $args['selector'] ) || ! isset( $args['render_callback'] ) || ! is_callable( $args['render_callback'] ) ) {
				/* translators: %1$s represents the field ID where the error occurs. */
				_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"partial_refresh" invalid entry in field %s', 'xclean' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_output() {
		...
		if ( ! is_array( $this->output ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'xclean' ), esc_html( $this->settings ) ), '3.0.10' );
			$this->output = array(
				array(
					'element' => $this->output,
				),
